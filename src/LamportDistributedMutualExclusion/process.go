...
// simulate the logical clock update
func (p *process) updateTSRegurlaly() {
	for {
		time.Sleep(time.Duration(rand.Intn((logicalClockIntervalHigh-logicalClockIntervalLow)+logicalClockIntervalLow)) * time.Millisecond)
		p.mu.Lock()
		p.TS += msgp3.TimeStamp(rand.Intn((logicalClockIncreHigh - logicalClockIncreLow) + logicalClockIncreLow))
		p.mu.Unlock()
	}
}
...
func (p *process) SetTS(ts msgp3.TimeStamp) {
	p.mu.Lock()
	if ts > p.TS {
		p.TS = ts
		p.logger.Printf("process(%v) update its local logical timeStamp to %v.\n", p.nodeID, ts)
	}
	p.TS = p.TS + 1
	p.mu.Unlock()
}
...
func (p *process) Run(w func(), msgContent string) error {
	...
	err = p.dl.Acquire(p.GetTS(), msgContent) // if any process still in critical section, it will block.
	// request failure
	if err != nil {
	...
	err = p.dl.Release(p.GetTS(), msgContent+"[Release]")
	if err != nil {

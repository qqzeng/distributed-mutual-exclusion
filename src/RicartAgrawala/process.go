...
func (p *process) updateTSRegurlaly() {
	for {
		...
		p.mu.Lock()
		p.TS += msgp2.TimeStamp(rand.Intn((logicalClockIncreHigh - logicalClockIncreLow) + logicalClockIncreLow))
		p.mu.Unlock()
	}
}

func (p *process) getTS() msgp2.TimeStamp {
	p.mu.Lock()
	defer p.mu.Unlock()
	return p.TS
}

func (p *process) SetTS(ts msgp2.TimeStamp) {
	p.mu.Lock()
	if ts > p.TS {
		p.TS = ts
